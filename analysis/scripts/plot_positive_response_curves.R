...
get_ks <- function(tr_names, class, p, alt_p, int = 0.1){
  ks <- NULL
  # if(class == "Site"){
    p_seq = seq(0,1,int)
  # } else if(class == "background"){
...
alt_p = 0.5
int = 0.1
ks_site <- get_ks(tr_names, "Site", p, alt_p, int)
ks_background <- get_ks(tr_names, "background", p, alt_p, int)
...
for(i in seq_along(alt_p_seq)){
  ks_site <- get_ks(tr_names, "Site", p, alt_p_seq[i], int)
  Site_pred <- 1 / (1 + exp(-as.vector(ks_site %*% alphas_pred)))
...
plot_dat2 <- data.frame(pred = Site_pred2$pred, 
                        ...
                        similarity = rep(alt_p_seq,length(alt_p_seq)),
                        class = rep(c("Site")))
